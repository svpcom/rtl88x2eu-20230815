/**************************************************************************************************************************
 *
 * RTL88x2EU userspace explicit SU TX beamforming for monitor mode
 * https://github.com/libc0607/rtl88x2eu-20230815
 * 
 * This re-implementation from scratch is a replacement of core/rtw_beamforming.c and hal/rtl8822e/rtl8822e_phy.c
 ...
 *
 *************************************************************************************************************************/
#define _RTL8822E_BF_MONITOR_C_
...
void bf_monitor_phydm_txbf_rfmode_8822e(PADAPTER adapter, u8 su_bfee_cnt)
{
	...
	if (su_bfee_cnt > 0 ) {
		...
		odm_set_rf_reg(dm, RF_PATH_B, RF_0xef, BIT(19), 0x0);
	        ...
	        // I don't know what here doing, but just fill in the initial values
	        
	  	/*Path A ==================*/
		/*RF mode table write enable*/
		odm_set_rf_reg(dm, RF_PATH_A, RF_0xef, BIT(19), 0x1);
		/*Select RX mode*/
		odm_set_rf_reg(dm, RF_PATH_A, RF_0x33, 0xF, 0xe);
		/*Set Table data*/
		odm_set_rf_reg(dm, RF_PATH_A, RF_0x3e, 0xF, 0xc);
		/*Set Table data*/
		odm_set_rf_reg(dm, RF_PATH_A, RF_0x3f, 0xfffff, 0x4);
		/*RF mode table write disable*/
		odm_set_rf_reg(dm, RF_PATH_A, RF_0xef, BIT(19), 0x0);
		...
		odm_set_rf_reg(dm, RF_PATH_B, RF_0xef, BIT(19), 0x1);
		/*Select RX mode*/
		odm_set_rf_reg(dm, RF_PATH_B, RF_0x33, 0xF, 0xe);
		/*Set Table data*/
		odm_set_rf_reg(dm, RF_PATH_B, RF_0x3e, 0xF, 0xc);
		/*Set Table data*/
		odm_set_rf_reg(dm, RF_PATH_B, RF_0x3f, 0xfffff, 0x4);
		/*Select Standby mode*/
		//odm_set_rf_reg(dm, RF_PATH_B, RF_0x33, 0xF, 1);
		/*Set Table data*/
		//odm_set_rf_reg(dm, RF_PATH_B, RF_0x3e, 0xF, 0x1);
...
)
{
	...
	txbf_ctrl &= ~(BIT_R_TXBF0_20M_8822E|BIT_R_TXBF0_40M_8822E|BIT_R_TXBF0_80M_8822E);
        if (bf_en) {
              switch (bw) {
	      default:
	      case CHANNEL_WIDTH_80:
		      txbf_ctrl |= BIT_R_TXBF0_80M_8822E;
		      fallthrough;
	      case CHANNEL_WIDTH_40:
		      txbf_ctrl |= BIT_R_TXBF0_40M_8822E;
		      fallthrough;
...
)
{
	...
	addr_csi_rpt = REG_TX_CSI_RPT_PARAM_BW20_8822E;

	/* Sounding protocol control */
	rtw_write8(adapter, REG_SND_PTCL_CTRL_8822E, 0xDB);
	...
	rtw_write8(adapter, REG_SND_PTCL_CTRL_8822E+3, 0x70);
	
	// BIT_CSI_GID_SEL_8822E
	tmp6dc = (rtw_read32(adapter, REG_BBPSF_CTRL_8822E) | BIT(30) | (csi_rsc << 13));
	tmp6dc = (g_id==63)? (tmp6dc|BIT(12)): (tmp6dc&(~BIT(12)));
	rtw_write32(adapter, REG_BBPSF_CTRL_8822E, tmp6dc);
	
	rtw_write32(adapter, REG_CSI_RRSR_8822E, 0x550);
}
...
)
{
	...
	bf_monitor_phydm_txbf_rfmode_8822e(adapter, 1);

	/* P_AID of Beamformee & enable NDPA transmission & enable NDPA interrupt */
	val32 = rtw_read32(adapter, REG_TXBF_CTRL_8822E);
	val32 = BIT_SET_R_TXBF0_AID_8822E(val32, p_aid);
	val32 &= ~(BIT_R_TXBF0_20M_8822E | BIT_R_TXBF0_40M_8822E | BIT_R_TXBF0_80M_8822E);
	val32 |= BIT_R_EN_NDPA_INT_8822E | BIT_USE_NDPA_PARAMETER_8822E | BIT_R_ENABLE_NDPA_8822E;
	rtw_write32(adapter, REG_TXBF_CTRL_8822E, val32);

	/* CSI report parameters of Beamformee */
	val32 = rtw_read32(adapter, REG_ASSOCIATED_BFMEE_SEL_8822E);
	val32 = BIT_SET_AID0_8822E(val32, p_aid);
	val32 |= BIT_TXUSER_ID0_8822E;

	/* unknown? */
	val32 &= 0x03FFFFFF;
	val32 |= 0x60000000;

	rtw_write32(adapter, REG_ASSOCIATED_BFMEE_SEL_8822E, val32);
}
...
void bf_monitor_sounding_reset(PADAPTER adapter)
{
	...
	rtw_write8(adapter, REG_TXBF_CTRL_8822E+3, 0);

	mu_tx_ctl = rtw_read32(adapter, REG_MU_TX_CTL_8822E);
	for (i=0; i<6; i++) {
		mu_tx_ctl = BIT_SET_R_MU_TAB_SEL_8822E(mu_tx_ctl, i);
		rtw_write32(adapter, REG_MU_TX_CTL_8822E, mu_tx_ctl);
		/* set MU STA gid valid table */
		rtw_write32(adapter, REG_MU_STA_GID_VLD_8822E, 0);
	}

	/* Disable TxMU PPDU */
	mu_tx_ctl &= ~BIT_R_EN_MU_MIMO_8822E;
	rtw_write32(adapter, REG_MU_TX_CTL_8822E, mu_tx_ctl);
}

void bf_monitor_reset_beamformer(PADAPTER adapter)
{
	rtw_write32(adapter, REG_ASSOCIATED_BFMER0_INFO_8822E, 0);
	rtw_write16(adapter, REG_ASSOCIATED_BFMER0_INFO_8822E+4, 0);
	rtw_write16(adapter, REG_TX_CSI_RPT_PARAM_BW20_8822E, 0);
...
void bf_monitor_reset_beamformee(PADAPTER adapter)
{
	...
	/* clear P_AID */
	txbf_ctrl = rtw_read32(adapter, REG_TXBF_CTRL_8822E);
	bfmee_sel = rtw_read32(adapter, REG_ASSOCIATED_BFMEE_SEL_8822E);

	txbf_ctrl = BIT_SET_R_TXBF0_AID_8822E(txbf_ctrl, 0);
	txbf_ctrl &= ~(BIT_R_TXBF0_20M_8822E | BIT_R_TXBF0_40M_8822E | BIT_R_TXBF0_80M_8822E);

	bfmee_sel = BIT_SET_AID0_8822E(bfmee_sel, 0);
	bfmee_sel &= ~BIT_TXUSER_ID0_8822E;

	txbf_ctrl |= BIT_R_EN_NDPA_INT_8822E | BIT_USE_NDPA_PARAMETER_8822E | BIT_R_ENABLE_NDPA_8822E;
	rtw_write32(adapter, REG_TXBF_CTRL_8822E, txbf_ctrl);
	rtw_write32(adapter, REG_ASSOCIATED_BFMEE_SEL_8822E, bfmee_sel);

	RTW_INFO("%s: Clear SU BFee entry 0 HW setting\n", __FUNCTION__);
}

void bf_monitor_phy_reset_rtl8822e(PADAPTER adapter)
{
	u8 i;
	u32 val32;
	...
	val32 = rtw_read32(adapter, REG_MU_TX_CTL_8822E);
	val32 = BIT_SET_R_MU_TABLE_VALID_8822E(val32, 0);
	rtw_write32(adapter, REG_MU_TX_CTL_8822E, val32);

	/* Reset SU BFer entry setting */
	rtw_write32(adapter, REG_ASSOCIATED_BFMER0_INFO_8822E, 0);
	rtw_write16(adapter, REG_ASSOCIATED_BFMER0_INFO_8822E+4, 0);
	rtw_write16(adapter, REG_TX_CSI_RPT_PARAM_BW20_8822E, 0);
	rtw_write32(adapter, REG_ASSOCIATED_BFMER1_INFO_8822E, 0);
	rtw_write16(adapter, REG_ASSOCIATED_BFMER1_INFO_8822E+4, 0);
	rtw_write16(adapter, REG_TX_CSI_RPT_PARAM_BW20_8822E+2, 0);

	/* Force disable sounding */
	bf_monitor_sounding_reset(adapter);

	/* Config RF mode */
	bf_monitor_phydm_txbf_rfmode_8822e(adapter, 0);

	/* Disable sending NDPA & BF-rpt-poll to all BFee */
	for (i=0; i < 6; i++)
		rtw_write16(adapter, REG_WMAC_ASSOCIATED_MU_BFMEE2_8822E+(i*2), 0);

	/* set validity of MU STA */
	rtw_write32(adapter, REG_MU_TX_CTL_8822E, 0);

	/* Reset SU BFee entry setting */
	/* SU BF0 and BF1 */
	val32 = BIT_R_EN_NDPA_INT_8822E | BIT_USE_NDPA_PARAMETER_8822E | BIT_R_ENABLE_NDPA_8822E;
	rtw_write32(adapter, REG_TXBF_CTRL_8822E, val32);
	rtw_write32(adapter, REG_ASSOCIATED_BFMEE_SEL_8822E, 0);
...
void bf_monitor_reg_init(PADAPTER adapter)
{
	...
	v32 = rtw_read32(adapter, REG_MU_TX_CTL_8822E);
	/* Enable P1 aggr new packet according to P0 transfer time */
	v32 |= BIT_R_MU_P1_WAIT_STATE_EN_8822E;
	/* MU Retry Limit */
	v32 = BIT_SET_R_MU_RL_8822E(v32, 0xA);
	/* Disable Tx MU-MIMO until sounding done */
	v32 &= ~BIT_R_EN_MU_MIMO_8822E;
	/* Clear validity of MU STAs */
	v32 = BIT_SET_R_MU_TABLE_VALID_8822E(v32, 0);
	rtw_write32(adapter, REG_MU_TX_CTL_8822E, v32);

	/* MU-MIMO Option as default value */
	v8 = BIT_WMAC_TXMU_ACKPOLICY_8822E(3);
	v8 |= BIT_WMAC_TXMU_ACKPOLICY_EN_8822E;
	rtw_write8(adapter, REG_MU_BF_OPTION_8822E, v8);
	/* MU-MIMO Control as default value */
	rtw_write16(adapter, REG_WMAC_MU_BF_CTL_8822E, 0);

	/* Set MU NDPA rate & BW source */
	/* 0x42C[30] = 1 (0: from Tx desc, 1: from 0x45F) */
	v8 = rtw_read8(adapter, REG_TXBF_CTRL_8822E+3);
	v8 |= (BIT_USE_NDPA_PARAMETER_8822E >> 24);
	rtw_write8(adapter, REG_TXBF_CTRL_8822E+3, v8);
	/* 0x45F[7:0] = 0x10 (Rate=OFDM_6M, BW20) */
	rtw_write8(adapter, REG_NDPA_OPT_CTRL_8822E, 0x10);
...
{
	rtw_write8(adapter, REG_SND_PTCL_CTRL_8822E, 0xD8);
...
void bf_monitor_reset(PADAPTER adapter)
{
    ...
    bf_monitor_phy_reset_rtl8822e(adapter);
...
void bf_monitor_print_conf(PADAPTER adapter, struct seq_file *m)
{
    ...
    RTW_PRINT_SEL(m, "REG_TXBF_CTRL_8822E.AID0: %u\n", BIT_GET_R_TXBF0_AID_8822E(txbf_ctrl));
    RTW_PRINT_SEL(m, "REG_TXBF_CTRL_8822E.EN_NDPA_INT: %u\n", txbf_ctrl & BIT_R_EN_NDPA_INT_8822E? 1: 0);
    RTW_PRINT_SEL(m, "REG_TXBF_CTRL_8822E.USE_NDPA_PARAMETER: %u\n", txbf_ctrl & BIT_USE_NDPA_PARAMETER_8822E? 1: 0);
